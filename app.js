...
(function () {
  ...
  function renderDashboard() {
    ...
    // «Все деньги, которые есть» — счета + чистый доход по операциям + оплаченные подработки + накопления по целям
    var totalBalance = accountsBalance + (allIncome - allExpense) + paidSidejobsEarned + totalSavings;
    ...
    if (incomeDeltaEl) incomeDeltaEl.textContent = monthTx.filter(function (t) { return t.type === 'income'; }).length + ' операций за месяц';
  ...
  function renderBalanceMiniChart() {
    ...
    var accountsBalance = sum(state.accounts, 'balance');
    var totalSavings = sum(state.goals, 'currentAmount');
    var paidSidejobs = sum(state.sidejobs.filter(function (j) { return j.status === 'paid'; }), 'amount');
    var base = accountsBalance + totalSavings + paidSidejobs;
    var cumulative = [];
    var running = base;
    ...
    if (cumulative.every(function (v) { return v === cumulative[0]; })) {
      cumulative = cumulative.map(function (v, i) { return v + i * 0.0001; });
    }
    drawSparkline(canvas, cumulative, '#C0A053');
  }

  function renderBalanceHistoryChart() {
    var keys = lastMonthKeys(6);
    var hasTx = state.transactions.length > 0;
    renderChartOrEmpty('chartBalanceHistory', '#chartBalanceHistoryCanvas', hasTx, function (canvas) {
      ...
      var totalSavings = sum(state.goals, 'currentAmount');
      var paidSidejobs = sum(state.sidejobs.filter(function (j) { return j.status === 'paid'; }), 'amount');
      var running = accountsBalance + totalSavings + paidSidejobs;
      var cumulative = keys.map(function (k, i) { running += income[i] - expense[i]; return running; });
      drawLineChart(canvas, keys.map(shortMonthLabel), [{ values: cumulative, color: '#2A8A67' }]);
    });
  }
  ...
  function renderAnalyticsCharts() {
    ...
    renderChartOrEmpty('chartIncomeDynamics', 'canvas', incomeByMonth.some(Boolean), function (c) {
      drawLineChart(c, monthLabels, [{ values: incomeByMonth, color: '#2A8A67' }]);
    });
    renderChartOrEmpty('chartExpenseDynamics', 'canvas', expenseByMonth.some(Boolean), function (c) {
      drawLineChart(c, monthLabels, [{ values: expenseByMonth, color: '#A6402C' }]);
    });
    renderChartOrEmpty('chartBalanceChange', 'canvas', hasAnyTx, function (c) {
      drawLineChart(c, monthLabels, [{ values: netByMonth, color: '#3E7CB1' }]);
    });
    ...
    renderChartOrEmpty('chartPillowDynamics', 'canvas', incomeByMonth.some(Boolean), function (c) {
      drawLineChart(c, monthLabels, [{ values: pillowByMonth, color: '#8C6D26' }]);
    });

    renderChartOrEmpty('chartSavingsDynamics', 'canvas', state.goals.length > 0, function (c) {
      drawBarChart(c, state.goals.map(function (g) { return g.name.length > 8 ? g.name.slice(0, 7) + '…' : g.name; }), [
        { values: state.goals.map(function (g) { return g.currentAmount || 0; }), color: '#2A8A67' },
        { values: state.goals.map(function (g) { return g.targetAmount || 0; }), color: '#DDE3DD' }
      ]);
    });
    ...
    renderChartOrEmpty('chartBalanceForecast', 'canvas', netByMonth.some(Boolean), function (c) {
      ...
      drawLineChart(c, extendedLabels, [
        { values: historySeries, color: '#3E7CB1' },
        { values: forecastSeries.map(function (v) { return v === null ? lastActual : v; }), color: '#C0A053', dashed: true }
      ]);
    });

    renderChartOrEmpty('chartGoalsForecast', 'canvas', state.goals.length > 0, function (c) {
      drawBarChart(c, state.goals.map(function (g) { return g.name.length > 8 ? g.name.slice(0, 7) + '…' : g.name; }), [
        { values: state.goals.map(function (g) { return g.targetAmount > 0 ? Math.round((g.currentAmount / g.targetAmount) * 100) : 0; }), color: '#C0A053' }
      ]);
    });
  }
  ...
  function initAnalyticsFilters() {
    var el = document.getElementById('analyticsPeriod');
    if (el) el.addEventListener('change', renderAnalyticsCharts);
  }
