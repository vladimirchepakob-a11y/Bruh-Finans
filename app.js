...
(function () {
  ...
  function renderSidejobs() {
    ...
    body.innerHTML = list.map(function (j) {
      ...
      var nextStatusHint = j.status === 'paid' ? 'Отметить как ожидает оплаты' : 'Отметить как оплачено';
      return '<tr>' +
        ...
        '<td>' +
          '<button type="button" class="status-pill status-pill--' + j.status + '" data-toggle-status="' + j.id + '" title="' + nextStatusHint + '" aria-label="' + nextStatusHint + '">' + statusLabel + '</button>' +
        '</td>' +
    ...
    body.querySelectorAll('[data-toggle-status]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var id = btn.getAttribute('data-toggle-status');
        var job = state.sidejobs.filter(function (j) { return j.id === id; })[0];
        if (!job) return;
        job.status = job.status === 'paid' ? 'pending' : 'paid';
        saveState();
        renderSidejobs();
        renderDashboard();
        toast(job.status === 'paid' ? 'Подработка отмечена как оплаченная' : 'Подработка отмечена как ожидающая оплаты');
        hapticFeedback('success');
      });
    });
  ...
  var CALENDAR_WEEKDAYS = ['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс'];

  function dayHasData(dateStr) {
    return state.transactions.some(function (t) { return t.date === dateStr; }) ||
      state.sidejobs.some(function (j) { return j.date === dateStr; });
  }
  ...
  function renderCalendar() {
    ...
    if (label) label.textContent = monthOptionLabel(y, m);

    var firstDay = new Date(y, m, 1);
    var startOffset = (firstDay.getDay() + 6) % 7; // делаем неделю с понедельника
    var daysInMonth = new Date(y, m + 1, 0).getDate();
    var todayStr = todayISO();

    var cells = CALENDAR_WEEKDAYS.map(function (w) {
      return '<div class="calendar-grid__weekday">' + w + '</div>';
    });

    for (var i = 0; i < startOffset; i++) {
      cells.push('<div class="calendar-grid__cell--empty"></div>');
    }

    for (var d = 1; d <= daysInMonth; d++) {
      var dateStr = y + '-' + String(m + 1).padStart(2, '0') + '-' + String(d).padStart(2, '0');
      var classes = [];
      if (dateStr === todayStr) classes.push('calendar-grid__cell--today');
      if (dateStr === calendarState.selectedDate) classes.push('calendar-grid__cell--active');
      var dot = dayHasData(dateStr) ? '<span class="calendar-grid__dot"></span>' : '';
      cells.push(
        '<div class="' + classes.join(' ') + '" data-date="' + dateStr + '">' +
        '<div class="calendar-grid__cell-inner"><span>' + d + '</span>' + dot + '</div>' +
        '</div>'
      );
    }
    ...
    grid.querySelectorAll('[data-date]').forEach(function (cell) {
      cell.addEventListener('click', function () {
        calendarState.selectedDate = cell.getAttribute('data-date');
        renderCalendar();
        renderCalendarDayDetails(calendarState.selectedDate);
      });
    });
  }

  function renderCalendarDayDetails(dateStr) {
    var titleEl = document.getElementById('calendarSelectedDate');
    var statsEl = document.getElementById('calendarDayStats');
    var eventsEl = document.getElementById('calendarDayEvents');
    if (!titleEl || !statsEl || !eventsEl) return;
    ...
    var dayJobs = state.sidejobs.filter(function (j) { return j.date === dateStr; });

    if (!dayTx.length && !dayJobs.length) {
      statsEl.innerHTML = '<p class="empty-state">Нет данных за выбранный день</p>';
      eventsEl.innerHTML = '';
      return;
    }

    var income = sum(dayTx.filter(function (t) { return t.type === 'income'; }), 'amount');
    var expense = sum(dayTx.filter(function (t) { return t.type === 'expense'; }), 'amount');
    var earned = sum(dayJobs, 'amount');

    statsEl.innerHTML =
      '<div class="report-summary">' +
      '<div class="report-summary__item"><span class="report-summary__label">Доходы</span><span class="report-summary__value amount-positive">' + formatMoney(income) + '</span></div>' +
      '<div class="report-summary__item"><span class="report-summary__label">Расходы</span><span class="report-summary__value amount-negative">' + formatMoney(expense) + '</span></div>' +
      '<div class="report-summary__item"><span class="report-summary__label">Подработки</span><span class="report-summary__value">' + formatMoney(earned) + '</span></div>' +
      '</div>';

    var items = [];
    dayTx.forEach(function (t) {
      var cls = t.type === 'income' ? 'amount-positive' : 'amount-negative';
      var sign = t.type === 'income' ? '+' : '−';
      items.push(
        '<li class="list__item"><span>' + escapeHtml(t.category) + (t.comment ? ' · ' + escapeHtml(t.comment) : '') + '</span>' +
        '<span class="' + cls + '">' + sign + ' ' + formatMoney(t.amount) + '</span></li>'
      );
    });
    dayJobs.forEach(function (j) {
      var statusLabel = j.status === 'paid' ? 'Оплачено' : 'Ожидает оплаты';
      items.push(
        '<li class="list__item"><span>' + escapeHtml(j.title) + ' · ' + statusLabel + '</span>' +
        '<span>' + formatMoney(j.amount) + '</span></li>'
      );
    });
  ...
  function getReportFilters() {
    return {
      ...
      type: (document.getElementById('reportType') || {}).value || 'full'
    };
  }
  ...
  function buildReport() {
    ...
    var jobs = state.sidejobs.filter(function (j) {
      return (!f.from || j.date >= f.from) && (!f.to || j.date <= f.to);
    });
    ...
    var totals = {
      ...
      sidejobs: sum(jobs, 'amount'),
      sidejobsPaid: sum(jobs.filter(function (j) { return j.status === 'paid'; }), 'amount')
    };
    totals.net = totals.income - totals.expense;

    var rows = [];
    if (f.type === 'full' || f.type === 'income') {
      income.forEach(function (t) {
        rows.push({ date: t.date, kind: 'Доход', category: t.category, comment: t.comment, amount: t.amount });
      });
    }
    if (f.type === 'full' || f.type === 'expense') {
      expense.forEach(function (t) {
        rows.push({ date: t.date, kind: 'Расход', category: t.category, comment: t.comment, amount: -t.amount });
      });
    }
    if (f.type === 'full' || f.type === 'sidejobs') {
      jobs.forEach(function (j) {
        var statusLabel = j.status === 'paid' ? 'Оплачено' : 'Ожидает оплаты';
        rows.push({ date: j.date, kind: 'Подработка', category: j.title, comment: statusLabel, amount: j.amount });
      });
    }
    rows.sort(function (a, b) { return a.date.localeCompare(b.date); });

    return { filters: f, rows: rows, totals: totals };
  }

  function renderReportPreview(report) {
    var emptyEl = document.getElementById('reportEmptyState');
    var contentEl = document.getElementById('reportContent');
    if (!emptyEl || !contentEl) return;

    if (!report.rows.length) {
      emptyEl.textContent = 'Нет данных за выбранный период';
    ...
    var summaryHtml =
      '<div class="report-summary">' +
      '<div class="report-summary__item"><span class="report-summary__label">Доходы</span><span class="report-summary__value amount-positive">' + formatMoney(report.totals.income) + '</span></div>' +
      '<div class="report-summary__item"><span class="report-summary__label">Расходы</span><span class="report-summary__value amount-negative">' + formatMoney(report.totals.expense) + '</span></div>' +
      '<div class="report-summary__item"><span class="report-summary__label">Итог</span><span class="report-summary__value">' + formatMoney(report.totals.net) + '</span></div>' +
      '<div class="report-summary__item"><span class="report-summary__label">Подработки (оплачено)</span><span class="report-summary__value">' + formatMoney(report.totals.sidejobsPaid) + '</span></div>' +
      '</div>';

    var rowsHtml = report.rows.map(function (r) {
      var cls = r.amount >= 0 ? 'amount-positive' : 'amount-negative';
      var sign = r.amount >= 0 ? '+' : '−';
      return '<tr>' +
        ...
        '<td class="table__align-right ' + cls + '">' + sign + ' ' + formatMoney(Math.abs(r.amount)) + '</td>' +
        '</tr>';
    }).join('');

    var tableHtml =
      '<div class="table-scroll"><table class="table"><thead><tr>' +
      '<th>Дата</th><th>Тип</th><th>Категория</th><th>Комментарий</th><th class="table__align-right">Сумма</th>' +
  ...
  function exportReportCsv(report) {
    var lines = [['Дата', 'Тип', 'Категория', 'Комментарий', 'Сумма'].join(';')];
    ...
    downloadBlob('\uFEFF' + lines.join('\r\n'), 'text/csv;charset=utf-8', 'otchet-' + todayISO() + '.csv');
    toast('CSV-файл скачан');
  }

  function exportReportJson(report) {
    downloadBlob(JSON.stringify(report, null, 2), 'application/json', 'otchet-' + todayISO() + '.json');
    toast('JSON-файл скачан');
  }

  function exportReportPdf(report) {
    var win = window.open('', '_blank');
    if (!win) { toast('Разрешите всплывающие окна, чтобы сохранить PDF', 'error'); return; }

    var rowsHtml = report.rows.map(function (r) {
      var sign = r.amount >= 0 ? '+' : '−';
      var color = r.amount >= 0 ? '#00B67A' : '#FF4D4D';
      return '<tr>' +
        '<td>' + formatDate(r.date) + '</td>' +
        '<td>' + escapeHtml(r.kind) + '</td>' +
        '<td>' + escapeHtml(r.category || '—') + '</td>' +
        '<td>' + escapeHtml(r.comment || '—') + '</td>' +
        '<td style="text-align:right;color:' + color + '">' + sign + ' ' + formatMoney(Math.abs(r.amount)) + '</td>' +
        '</tr>';
    }).join('');

    var periodLabel = (report.filters.from ? formatDate(report.filters.from) : 'начало') + ' — ' + (report.filters.to ? formatDate(report.filters.to) : 'сегодня');

    var html = '<!doctype html><html lang="ru"><head><meta charset="utf-8"><title>Отчет Finance Pro</title>' +
      '<style>' +
      'body{font-family:Arial,Helvetica,sans-serif;padding:28px;color:#14151A}' +
      'h1{font-size:20px;margin-bottom:4px}' +
      'p.period{color:#4B4C55;margin-top:0}' +
      'table{width:100%;border-collapse:collapse;margin-top:18px}' +
      'th,td{padding:8px 10px;border-bottom:1px solid #E7E7EC;font-size:13px;text-align:left}' +
      'th{color:#86878F;font-weight:700}' +
      '.totals{display:flex;gap:24px;margin-top:14px;flex-wrap:wrap}' +
      '.totals div{font-size:13px}' +
      '.totals b{display:block;font-size:16px}' +
      '@media print { body{padding:0} }' +
      '</style></head><body>' +
      '<h1>Отчет Finance Pro</h1>' +
      '<p class="period">Период: ' + periodLabel + '</p>' +
      '<div class="totals">' +
      '<div>Доходы<b style="color:#00B67A">' + formatMoney(report.totals.income) + '</b></div>' +
      '<div>Расходы<b style="color:#FF4D4D">' + formatMoney(report.totals.expense) + '</b></div>' +
      '<div>Итог<b>' + formatMoney(report.totals.net) + '</b></div>' +
      '<div>Подработки (оплачено)<b>' + formatMoney(report.totals.sidejobsPaid) + '</b></div>' +
      '</div>' +
      '<table><thead><tr><th>Дата</th><th>Тип</th><th>Категория</th><th>Комментарий</th><th style="text-align:right">Сумма</th></tr></thead><tbody>' + rowsHtml + '</tbody></table>' +
      '</body></html>';
  ...
  function initReports() {
    bind('generateReportBtn', 'click', function () {
      lastReport = buildReport();
      renderReportPreview(lastReport);
      toast('Отчет сформирован');
      hapticFeedback('success');
    });
    bind('exportReportCsv', 'click', function () {
      if (!lastReport) { toast('Сначала сформируйте отчет', 'error'); return; }
      exportReportCsv(lastReport);
    });
    bind('exportReportJson', 'click', function () {
      if (!lastReport) { toast('Сначала сформируйте отчет', 'error'); return; }
      exportReportJson(lastReport);
    });
    bind('exportReportPdf', 'click', function () {
      if (!lastReport) { toast('Сначала сформируйте отчет', 'error'); return; }
